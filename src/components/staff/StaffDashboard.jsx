...
const StaffDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 w-64 bg-blue-800 text-white transform transition-transform duration-300 ease-in-out z-40 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      } lg:translate-x-0`}>
        <div className="p-4">
          <h2 className="text-2xl font-bold mb-8 mt-8 lg:mt-0">Staff Dashboard</h2>
          <nav>
            {menuItems.map((item) => (
              <button
                key={item.id}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-blue-900">
          {userInfo && (
            <div className="mb-4">
              ...
              <p className="text-xs text-blue-300">Staff</p>
            </div>
          )}
          <button
      ...
      <div className="lg:ml-64 p-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Quản lý hệ thống phòng chống ma túy</p>
        </div>
        ...
        {activeTab === "appointments" && <AppointmentSupport />}
      </div>
    </div>
  );
};
