    ...
    <div className="flex min-h-screen">
        ...
        <div>
          ...
          <nav className="space-y-2 px-4">
            <button 
              ...
              className={`w-full text-left px-4 py-2 rounded ${activeSection === 'appointments' ? 'bg-blue-800' : 'hover:bg-blue-800'}`}
            >
              📅 Lịch hẹn
            </button>
            <button 
              onClick={() => setActiveSection('profile')} 
              className={`w-full text-left px-4 py-2 rounded ${activeSection === 'profile' ? 'bg-blue-800' : 'hover:bg-blue-800'}`}
            >
              👤 Hồ sơ
            </button>
          </nav>
      ...
      <main className="flex-1 p-4 md:p-8 bg-gray-100">
        ...
        <div className="grid gap-6">
          {activeSection === 'appointments' && <ConsultantAppointments />}
          {activeSection === 'profile' && <ConsultantProfile />}
        </div>
      </main>
    </div>
  );
